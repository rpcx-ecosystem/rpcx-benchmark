...
func main() {
	...
	for i := 0; i < n; i++ {
		...
		go func(i int) {
			...
			client := goframe.NewLengthFieldBasedFrameConn(encoderConfig, decoderConfig, conn)
			...
			//warmup
			for j := 0; j < 5; j++ {
				n, _ := args.MarshalTo(data)
				err := client.WriteFrame(data[:n])
				if err != nil {
					fmt.Println(err)
				}
			}
			...
			for j := 0; j < m; j++ {
				...
				err := client.WriteFrame(data[:n])
				if err == nil {
					replyData, err = client.ReadFrame()
					if err == nil {
						err = reply.Unmarshal(replyData)
					}
				}
