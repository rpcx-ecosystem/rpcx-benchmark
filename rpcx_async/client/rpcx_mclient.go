...
func main() {
	...
	for i := 0; i < n; i++ {
		...
		go func(i int) {
			...
			ch := make(chan *CallWithTime, 1000)
			//send
			go func() {
				for j := 0; j < m; j++ {
					t := time.Now().UnixNano()
					call, err := xclient.Go(context.Background(), serviceMethod, args, &reply, nil)
			...
			for cwt := range ch {
				...
				<-cwt.call.Done
				d[i] = append(d[i], time.Now().UnixNano()-cwt.start)
				if cwt.call.Reply.(*proto.BenchmarkMessage).Field1 == "OK" {
