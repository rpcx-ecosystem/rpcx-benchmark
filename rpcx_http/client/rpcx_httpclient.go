	...
	for i := 0; i < n; i++ {
		...
		go func(i int) {
			...
			for j := 0; j < m; j++ {
				...
				req, err := http.NewRequest("POST", "http://"+*host+"/", bytes.NewReader(data))
				if err != nil {
				...
				h := req.Header
				h.Set(gateway.XMessageID, strconv.Itoa(j))
				h.Set(gateway.XMessageType, "0")
				h.Set(gateway.XSerializeType, "2")
				h.Set(gateway.XServicePath, servicePath)
				h.Set(gateway.XServiceMethod, serviceMethod)
